...
//	int thumbCount = [dataSource_ thumbsViewNumberOfThumbs:self];
	...
	for (int index = startAtIndex; index < stopAtIndex; index++) {
		...
		if (isThumbViewMissing) {
			...
			KTThumbView *thumbView = [[KTThumbView alloc] initWithFrame:CGRectZero];
			
			if (![[photos objectAtIndex:index] underlyingImage]) {
				[[photos objectAtIndex:index] loadUnderlyingImageAndNotify];
			}
			
			[thumbView setController:controller_];
			[thumbView setImage:[[photos objectAtIndex:index] underlyingImage] forState:UIControlStateNormal];
